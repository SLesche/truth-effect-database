extract_from_submission_json <- function(json_path){
  ...
  for (istudy in 1:n_studies){
    ...
    columns_to_extract = c("subject", "session", "trial", "response", "repeated")
    
    # Here check if has conditions
    if (json_obj$study_info[[istudy]]$condition_data$has_within_conditions == "1"){
      submission_list$study_info[[istudy]]$within_data = data.table::rbindlist(json_obj$study_info[[istudy]]$condition_data$within_condition_details)
    ...
    if (json_obj$study_info[[istudy]]$condition_data$has_between_conditions == "1"){
      submission_list$study_info[[istudy]]$between_data = data.table::rbindlist(json_obj$study_info[[istudy]]$condition_data$between_condition_details)
    ...
    if (json_obj$study_info[[istudy]]$measurement_data$additional_measures == "1"){
      submission_list$study_info[[istudy]]$measurement_data = data.table::rbindlist(json_obj$study_info[[istudy]]$measurement_data$measures)
    }
  }
